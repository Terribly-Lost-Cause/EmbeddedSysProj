...
int main() {
    ...
    while (true) {
        ...
        if (dt >= LOOP_INTERVAL_MS) {
            ...
            if (fabs(heading_error) < 2.0f) heading_error = 0;
            ...
            if (left_pwm_current > 100) left_pwm_current = 100;

            if (left_pwm_current < 0)   left_pwm_current = 0;

            if (right_pwm_current > 100) right_pwm_current = 100;

            if (right_pwm_current < 0)   right_pwm_current = 0;
